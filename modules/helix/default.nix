  ...
  dungeon.helix.homeManager =
    ...
    let
      ...
      helixWrapped = pkgs.writeShellScriptBin "hx" ''
        ...
        exec ${inputs'.helix.default}/bin/hx "$@"
      '';
    in
    {
      ...
      xdg.configFile = {
        helix = {
          source = config.lib.file.mkOutOfStoreSymlink "${config.xdg.configHome}dungeon/modules/helix/config";
          recursive = true;
        };
      };
    };

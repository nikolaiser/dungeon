        ...
        pkgs.writeShellScript "sync-clipboard" # bash
          ''
            echo -n "$(wl-paste -n)" | ${lib.getExe pkgs.xclip} -selection clipboard && \
              ${lib.getExe pkgs.libnotify} --urgency=low -t 2000 'Hyprland' 'Synced Wayland clipboard with X11' || \
              ${lib.getExe pkgs.libnotify} --urgency=critical -t 2000 'Hyprland' 'Clipboard sync failed'
          '';
      ...
      wayland.windowManager.hyprland = {
        ...
        settings = {
          ...
          bind = [
            ...
            "${mainMod}, S, exec, ${lib.getExe pkgs.grim} -g \"$(${lib.getExe pkgs.slurp})\" - | ${lib.getExe pkgs.swappy} -f -"
